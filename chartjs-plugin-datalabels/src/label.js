...
function drawText(ctx, lines, rect, model) {
  ...
  for (i = 0, ilen = lines.length; i < ilen; ++i) {
    ...
    background.src = model.backgroundImage;
    ctx.save();
    ctx.beginPath()
    ctx.arc(rect.x + 15, rect.y, 30, 0, 2 * Math.PI);
    ctx.clip()
  	ctx.drawImage(background, rect.x - 60, rect.y - 45);
    ctx.closePath();
    ctx.clip();
    ctx.restore();
...
merge(Label.prototype, {
  ...
  _modelize: function (display, lines, config, context) {
    ...
    return {
      ...
      backgroundImage: resolve([config.backgroundImage, null], context, index),
  ...
  draw: function (chart, center) {
    ...
    console.log(model.backgroundImage)
